...
namespace Ordenador_de_numeros
{
    class Insercion
    {
        ...
        public Insercion() // metodo constructor para ejecutar
        {
            Console.Clear();
            Console.WriteLine("METODO DE ORDENAMIENTO DE INSERCION \n\n"); // para que el usuario sepa en que metodo está
        }
        ...
        public void ordenarNumeros()// el metodo de ordenamiento indicado, en este caso Insercion, para encontrar los metodos de ordenamiento utilice un foro en internet "jfprogramacionnet.blogspot.com"
        {
            int a;
            int j;
            for(int i=0; i < this.Numero.Length; i++)
            {
                a = this.Numero[i];
                j = i - 1;
                while (j>=0 && this.Numero[j] > a)
                {
                    this.Numero[j + 1] = this.Numero[j];
                    j--;
                }
                this.Numero[j + 1] = a;
            }
